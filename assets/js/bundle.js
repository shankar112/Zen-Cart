// bundle.js — classic script fallback (no ESM needed)
(() => {
  const qs=(s,r=document)=>r.querySelector(s), qsa=(s,r=document)=>[...r.querySelectorAll(s)], on=(t,e,h,o=false)=>e&&e.addEventListener(t,h,o);
  const debounce=(fn,wait=250)=>{let t;return(...a)=>{clearTimeout(t);t=setTimeout(()=>fn(...a),wait)}};
  const money=(n)=>`$${(n/100).toFixed(2)}`; const uid=()=>Math.random().toString(36).slice(2,9);
  const storage={get:(k,d=null)=>{try{const v=localStorage.getItem('zc_'+k);return v?JSON.parse(v):d}catch{return d}},set:(k,v)=>localStorage.setItem('zc_'+k,JSON.stringify(v))};
  ...
  const card=(p)=>tmpl(`<article class="card glass fade-up shine"><div class="thumb">${svg(p.img)}</div><div class="content"><div class="row between"><h3 class="mb-0" style="font-size:16px">${p.name}</h3><span class="price">${money(p.price)}</span></div><div class="row mt-1"><span class="chip">${p.category}</span><span class="rating">★ ${p.rating}</span></div><div class="row mt-2 between"><a class="btn ghost" href="pdp.html?id=${p.id}">View</a><button class="btn primary add" data-id="${p.id}">Add to cart</button></div></div></article>`);
  ...
  const addItem=(id,qty=1,price=null,name='')=>{qty=Math.max(1,Number(qty)||1);const c=get();const idx=c.findIndex(i=>i.id===id); if(idx>=0)c[idx].qty+=qty; else c.push({id,qty,price,name}); set(c)};
  ...
  const hydrate=(resolver)=>{const c=get();let ch=false;const n=c.map(i=>{if(i.price==null){const r=resolver(i.id); if(r){ch=true; return {...i,price:r.price,name:r.name}} return i} return i}); if(ch)set(n)};
  const lineTotal=(i)=>(Number(i.price)||0)*i.qty;
  const totals=()=>{const items=get();const subtotal=items.reduce((s,i)=>s+lineTotal(i),0);const shipping=subtotal>=FREE||subtotal===0?0:SHIP;const tax=Math.round(subtotal*TAX);const total=subtotal+shipping+tax;return{subtotal,shipping,tax,total,count:cartCount()}};

  // UI Helpers
  const updateBadge=()=>setText('#cart-count', String(cartCount()));
  const renderFeatured=()=>{const host=qs('#featured'); if(!host) return; host.innerHTML=''; PRODUCTS.slice(0,4).forEach(p=>host.appendChild(card(p))); host.addEventListener('click',e=>{const b=e.target.closest('.add'); if(!b)return; const p=getProductById(b.dataset.id); addItem(p.id,1,p.price,p.name); updateBadge();});};
  const bindPLP=()=>{const grid=qs('#plp'); if(!grid) return; const render=(arr)=>{grid.innerHTML=''; arr.forEach(p=>grid.appendChild(card(p)))}; render(PRODUCTS); const s=qs('#search'); const clr=qs('#clear'); s&&on('input',s,debounce(e=>render(searchProducts(e.target.value)),200)); clr&&on('click',clr,()=>{s.value=''; s.dispatchEvent(new Event('input'))}); grid.addEventListener('click',e=>{const b=e.target.closest('.add'); if(!b)return; const p=getProductById(b.dataset.id); addItem(p.id,1,p.price,p.name); updateBadge();}); };

  const initPDP=()=>{const name=qs('#name'); if(!name) return; const p=getProductById(params().id); if(!p){qs('main').innerHTML='<p>Product not found.</p>'; return;} setText('#bc-name',p.name); setText('#name',p.name); setText('#desc',p.desc); setText('#cat',p.category); setText('#rate',`★ ${p.rating}`); setText('#price',money(p.price)); qs('#img').innerHTML=`<div class="thumb" style="aspect-ratio:4/3">${svg(p.img)}</div>`; const q=qs('#qty'); on('click',qs('#inc'),()=>q.value=String((Number(q.value)||1)+1)); on('click',qs('#dec'),()=>q.value=String(Math.max(1,(Number(q.value)||1)-1))); on('click',qs('#add'),()=>{addItem(p.id,Number(q.value)||1,p.price,p.name); location.href='cart.html'}); };
